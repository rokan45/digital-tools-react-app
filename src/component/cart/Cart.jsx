...
const Cart = ({ addProduct, setAddProduct }) => {
    ...
    const handleProccedBtn = () => {
        // const removeAll = addProduct.filter((item) => item.id !== addProduct.id)
        setAddProduct([]);
            ...
            {addProduct.length === 0 ?
                ...
                <div>
                    <div className='mx-10 py-10 px-5 bg-gray-10 shadow-lg rounded-2xl'>
                        ...
                        <button onClick={() => handleProccedBtn()} className='btn w-full rounded-4xl bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white '>Proceed to Checkout</button>
